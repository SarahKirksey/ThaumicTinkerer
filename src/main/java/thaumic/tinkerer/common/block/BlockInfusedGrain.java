...
public class BlockInfusedGrain extends BlockCrops implements ITTinkererBlock {
  ...
  public static void setAspect(IBlockAccess world, int x, int y, int z,
                               Aspect aspect) {
  ...
  public IIcon getIcon(IBlockAccess world, int x, int y, int z, int meta) {
    if (meta < 7) {
      ...
      return this.icons[getNumberFromAspectForTexture(
          getAspect(world, x, y, z))][meta >> 1];
    } else {
      return this
          .icons[getNumberFromAspectForTexture(getAspect(world, x, y, z))][3];
    }
  }
  ...
  public void registerBlockIcons(IIconRegister par1IconRegister) {
    icons = new IIcon[7][4];
    String[] names = {"aer",  "ignis",    "aqua",   "terra",
                      "ordo", "perditio", "generic"};
    for (int j = 0; j < names.length; j++) {
      String s = names[j];
      for (int i = 0; i < 4; i++) {
        icons[j][i] =
            IconHelper.forName(par1IconRegister, "crop_" + s + "_" + i);
      }
    }
  }
  ...
  public int getPrimalTendencyCount(World world, int x, int y, int z,
                                    Aspect aspect) {
    return world.getTileEntity(x, y, z) instanceof TileInfusedGrain
        ? ((TileInfusedGrain)world.getTileEntity(x, y, z))
              .primalTendencies.getAmount(aspect)
        : 0;
  }
    ...
    if (metadata >= 7) {
      do {
        if (world.getTileEntity(x, y - 1, z) instanceof TileInfusedFarmland) {
          ...
          ((TileInfusedFarmland)world.getTileEntity(x, y - 1, z))
              .aspectList.add(currentAspect, 1);
          ((TileInfusedFarmland)world.getTileEntity(x, y - 1, z))
              .reduceSaturatedAspects();
          world.markBlockForUpdate(x, y - 1, z);
        }
      } while (world.rand.nextInt(55) <
               getPrimalTendencyCount(world, x, y, z, Aspect.EARTH));
    }
  }
  ...
  public Aspect getAspectDropped(World world, int x, int y, int z,
                                 int metadata) {
    Aspect currentAspect = getAspect(world, x, y, z);
    if (metadata >= 7 && currentAspect != null) {
      if (world.getTileEntity(x, y - 1, z) instanceof TileInfusedFarmland) {
        AspectList farmlandAspectList =
            ((TileInfusedFarmland)world.getTileEntity(x, y - 1, z)).aspectList;
        for (Aspect aspect : farmlandAspectList.getAspects()) {
          ...
          if (rand.nextInt(BREEDING_CHANCE) <
              (getPrimalTendencyCount(world, x, y, z, Aspect.FIRE) + 1) *
                  ...
                  farmlandAspectList.getAmount(aspect)) {
            if (ResearchManager.getCombinationResult(aspect, currentAspect) !=
                null) {
              return ResearchManager.getCombinationResult(aspect,
                                                          currentAspect);
            }
          }
        }
